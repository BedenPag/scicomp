...
def solve_euler_rk4(f, x0, t0, deltat_max, step_function, true_sol=None):
    ...
    if len(x0) == 2:
        ...
        # Plot the solution for x using the specified step function
        plt.plot(t_values, x_values, 'o', label=step_function.__name__)
        # Plot the true solution for x
        if true_sol is not None:
            plt.plot(domain, true_x, label='True solution')
        plt.xlabel('t')
        plt.ylabel('x')
        plt.legend()
        ...
        raise ValueError('Invalid number of dimensions. The function can only solve for 1 or 2 dimensions.')
